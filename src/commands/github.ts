		...
		.addFields(
			{
				...
				value: response.data.public_repos.toString(),
				inline: true,
			},
			{
				name: "Followers",
				value: response.data.followers.toString(),
				inline: true,
			},
			{
				name: "Following",
				value: response.data.following.toString(),
				inline: true,
			},
			{
...
export async function getRepos(username: string): Promise<EmbedBuilder> {
	...
	const embed = new EmbedBuilder()
		...
		.setDescription(
			response.data
				.map((repo: Repo) => `[${repo.name}](${repo.html_url})`)
				.join("\n"),
		)
		.setColor("#0099ff");
